...
public class RacingGameActivity extends Activity {
    ...
    public void start_clock(){
        new Thread() {
            public void run() {
                while(!isEnded&&remaining_time>0) {
                    try {
                        current_time=new Date().getTime();
                        remaining_time=240000-(current_time-start_time);
                        final long minute=remaining_time/1000/60;
                        final long second=remaining_time/1000%60;
    ...
    public void init_fields(){
        ...
        stop_button=(Button)findViewById(R.id.clear_button);
            ...
            public boolean onTouch(View v, MotionEvent event) {
                if(event.getAction() == MotionEvent.ACTION_DOWN){
                    ...
                    if(isEnded) {
                        ...
                        intent.setClass(RacingGameActivity.this,WelcomeActivity.class);
        ...
        save_button.setOnTouchListener(new View.OnTouchListener() {
            ...
            public boolean onTouch(View v, MotionEvent event) {
                if(event.getAction() == MotionEvent.ACTION_DOWN){
                    ...
                    db.execSQL("INSERT INTO Save VALUES('"+who_is_logining+"',"+score+","+(current_time-start_time)+",'"+content+"','racing')");
                    Toast.makeText(RacingGameActivity.this, "存档成功！" , Toast.LENGTH_SHORT).show();
                    Intent intent=new Intent();
                    intent.setClass(RacingGameActivity.this,WelcomeActivity.class);
                    startActivity(intent);
                }
    ...
    public void last_work(){
        ...
        db.execSQL("INSERT INTO RacingRank VALUES('"+who_is_logining+"','"+name+"',0,"+score+",'"+(current_time-start_time)+"','"+date+"','"+content+"')");
        cursor=db.rawQuery("SELECT userName,content FROM RacingRank ORDER BY score DESC",null);
        int index=1;
        while(cursor.moveToNext()){
            String userName=cursor.getString(0);
            String content_temp=cursor.getString(1);
            db.execSQL("UPDATE RacingRank SET rank="+index+" WHERE userName='"+userName+"' AND content='"+content_temp+"'");
            index++;
        }

        if((index-1)>9){
            db.execSQL("DELETE FROM RacingRank WHERE rank=10");
        }

        db.execSQL("INSERT INTO PlayHistory VALUES('"+who_is_logining+"',"+score+",'racing','"+(current_time-start_time)+"','"+date+"')");
