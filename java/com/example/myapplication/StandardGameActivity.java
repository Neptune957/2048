...
public class StandardGameActivity extends Activity {
            ...
            public boolean onTouch(View v, MotionEvent event) {
                if(event.getAction() == MotionEvent.ACTION_DOWN){
                    ...
                    if(isEnded) {
                        ...
                        intent.setClass(StandardGameActivity.this,WelcomeActivity.class);
        ...
        save_button.setOnTouchListener(new View.OnTouchListener() {
            ...
            public boolean onTouch(View v, MotionEvent event) {
                if(event.getAction() == MotionEvent.ACTION_DOWN){
                    ...
                    db.execSQL("INSERT INTO Save VALUES('"+who_is_logining+"',"+score+","+(current_time-start_time)+",'"+content+"','standard')");
                    Toast.makeText(StandardGameActivity.this, "存档成功！" , Toast.LENGTH_SHORT).show();
                    Intent intent=new Intent();
                    intent.setClass(StandardGameActivity.this,WelcomeActivity.class);
                    startActivity(intent);
                }
    ...
    public void last_work(){
        ...
        db.execSQL("INSERT INTO StandardRank VALUES('"+who_is_logining+"','"+name+"',0,"+score+",'"+(current_time-start_time)+"','"+date+"','"+content+"')");
        cursor=db.rawQuery("SELECT userName,content FROM StandardRank ORDER BY score DESC",null);
        int index=1;
        while(cursor.moveToNext()){
            String userName=cursor.getString(0);
            String content_temp=cursor.getString(1);
            db.execSQL("UPDATE StandardRank SET rank="+index+" WHERE userName='"+userName+"' AND content='"+content_temp+"'");
            index++;
        }

        if((index-1)>9){
            db.execSQL("DELETE FROM StandardRank WHERE rank=10");
        }

        db.execSQL("INSERT INTO PlayHistory VALUES('"+who_is_logining+"',"+score+",'standard','"+(current_time-start_time)+"','"+date+"')");
